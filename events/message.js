  ...
  if (message.channel.id === "738784380130033776") { // Verification Text Channel
    ...
    if (message.content.startsWith("resend")) {
      ...
      await message.author.send(dm).catch(() => {
        return message.reply("Your DMs are still locked. Please allow direct messages from this server!")
    ...
    if (!client.config.owners.includes(message.author.id)) { // The owner of the bot cannot get any verification codes.
      if (!message.author.bot) { // If the user was a robot, well return it.
        ...
        if (verify !== code) {
          ...
          message.delete()
          return message.reply("Boi are you sure you can read correctly cuz thats not the right code").then(i => i.delete({timeout: 10000}));
        }
        
        if (verify === code) {
          message.delete();
          db.delete(`verification.${message.author.id}`);
          message.reply("Thank you for verifying. You will now be given the Members role").then(i => i.delete({timeout: 7500}));
          setTimeout(function() {
            message.member.roles.add("738784118271246478");
